from controllers import conflict_controller
...
def conflicts(user):
    ...
    for conflict in conflict_controller.conflicts():
        st.html(
        ...
        if c1.button("Manter A", key=f"a_{conflict['id']}"):
            conflict_controller.resolve_conflict(conflict["id"], "manteve_a")
            set_toast("Conflito resolvido no mock.")
            st.rerun()
        if c2.button("Manter B", key=f"b_{conflict['id']}"):
            conflict_controller.resolve_conflict(conflict["id"], "manteve_b")
            set_toast("Conflito resolvido no mock.")
            st.rerun()
        if c3.button("Alterar", key=f"alt_{conflict['id']}"):
            conflict_controller.resolve_conflict(conflict["id"], "alterar")
